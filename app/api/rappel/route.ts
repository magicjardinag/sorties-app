...
export async function POST(request: Request) {
  try {
    const { email, titre, ville, quand, heure, prix, evenement_id, image_url, lat, lng, description, jours_avant } = await request.json()

    const delai = jours_avant || 1
    ...
    const dateEvenement = new Date(quand)
    const dateEnvoi = new Date(dateEvenement)
    dateEnvoi.setDate(dateEnvoi.getDate() - delai)

    // Enregistrer en BDD avec le délai et la date d'envoi
    await supabase.from("rappels").upsert({
      email,
      evenement_id,
      quand,
      jours_avant: delai,
      ...
      envoye: false,
    }, { onConflict: "email,evenement_id" })

    // Email de confirmation immédiate
    const delaiTexte = delai === 1 ? "la veille" : delai === 3 ? "3 jours avant" : "7 jours avant"
    ...
    await resend.emails.send({
      ...
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; background: #fff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 24px rgba(0,0,0,0.08);">
          ${image_url ? `<a href="${lienEvenement}"><img src="${image_url}" alt="${titre}" style="width: 100%; height: 220px; object-fit: cover; display: block;"/></a>` : ""}
          <div style="padding: 24px;">
            <p style="color: #7c3aed; font-weight: bold; font-size: 13px; margin: 0 0 8px;">✅ Rappel activé — ${delaiTexte} !</p>
            ...
            <div style="display: flex; gap: 12px; margin-bottom: 20px;">
              <a href="${lienEvenement}" style="flex: 1; background: #7c3aed; color: white; text-decoration: none; padding: 14px; border-radius: 12px; text-align: center; font-weight: bold; font-size: 15px; display: block;">Voir l'événement →</a>
              <a href="${lienMaps}" style="flex: 1; background: #f0fdf4; color: #16a34a; border: 1px solid #bbf7d0; text-decoration: none; padding: 14px; border-radius: 12px; text-align: center; font-weight: bold; font-size: 15px; display: block;">🗺️ Y aller</a>
            </div>
            <p style="color: #9ca3af; font-size: 13px; text-align: center; margin: 0;">
              Tu recevras un rappel <strong>${delaiTexte}</strong> de l'événement.<br/>
